...
/**
 * @title INSRegistryIgra
 * @notice Permanent .igra name registry for the Igra Network.
 *         Sister contract to INSRegistry (.ins) and INSRegistryIkas (.ikas) —
...
contract INSRegistryIgra {
    ...
    /* ─────────────────────────── State ──────────────────────────── */
    string public constant name = "Igra Name Service (.igra)";
    string public constant symbol = "INSG";
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name":"', label, '.igra",',
                '"description":"Permanent .igra name on the Igra Name Service. One-time payment, no renewals.",',
                '"image":"data:image/svg+xml;base64,', Base64Igra.encode(bytes(svg)), '",',
                '"attributes":[',
        ...
        return string(abi.encodePacked("data:application/json;base64,", Base64Igra.encode(bytes(json))));
    }
    ...
    /// @dev Renders a 500x500 SVG card with gradient, label, ".igra" suffix,
    ...
    function _renderSVG(string memory label) private pure returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                '<text x="468" y="51" font-family="Inter,system-ui,sans-serif" font-size="13" font-weight="700" fill="#e8e8f5" text-anchor="end" letter-spacing="2">INS-IGRA</text>',
                // label center
                '<text x="250" y="260" font-family="Inter,system-ui,sans-serif" font-size="', _toString(fontSize), '" font-weight="800" fill="#ffffff" text-anchor="middle">',
                    label,
                '</text>',
                // .igra suffix directly below
                '<text x="250" y="310" font-family="Inter,system-ui,sans-serif" font-size="26" font-weight="600" fill="url(#stroke)" text-anchor="middle">.igra</text>',
                // footer
...
/// @title Base64Igra
/// @notice Minimal Base64Igra encoder — OpenZeppelin-style (memory-local table).
library Base64Igra {

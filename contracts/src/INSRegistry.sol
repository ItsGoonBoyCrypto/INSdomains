...
/**
 * @title INSRegistry
 * @notice Permanent .ins name registry for the Igra Network.
...
contract INSRegistry {
    ...
    /* ─────────────────────────── State ──────────────────────────── */
    string public constant name = "Igra Name Service";
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name":"', label, '.ins",',
                '"description":"Permanent .ins name on the Igra Name Service. One-time payment, no renewals.",',
    ...
    /// @dev Renders a 500x500 SVG card with gradient, label, ".ins" suffix,
    ...
    function _renderSVG(string memory label) private pure returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                '<text x="468" y="51" font-family="Inter,system-ui,sans-serif" font-size="13" font-weight="700" fill="#e8e8f5" text-anchor="end" letter-spacing="2">INS</text>',
                // label center
                '<text x="250" y="260" font-family="Inter,system-ui,sans-serif" font-size="', _toString(fontSize), '" font-weight="800" fill="#ffffff" text-anchor="middle">',
                    label,
                '</text>',
                // .ins suffix directly below
                '<text x="250" y="310" font-family="Inter,system-ui,sans-serif" font-size="26" font-weight="600" fill="url(#stroke)" text-anchor="middle">.ins</text>',
                // footer
...
/// @title Base64
/// @notice Minimal Base64 encoder — OpenZeppelin-style (memory-local table).
library Base64 {

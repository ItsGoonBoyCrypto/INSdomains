...
| **INSResolverV2 (hardened, ENS-compatible)** | `0xcb2A450784849b85A797998EE220dC43d8B3f557` | 2026-05-18 | **Current resolver.** Trustless node→label binding — `cacheNode(label)` derives the namehash on-chain so poisoning is impossible. Expiry-aware `addr()`, owner-gated `setText()`. No owner / no admin. Points at V2 Registry. Verified on Blockscout. Replaces the original Resolver (left in place but no longer the recommended target). |
| INSResolver (original, ENS-compatible) | `0x451D84002cE0eCFd4cc622c72FA40849a8Bb5f2A` | 2026-04-23 | **DEPRECATED** — superseded by INSResolverV2 (2026-05-18) after a namehash-poisoning issue in its permissionless `cacheNode(label, node)`. Not used in any live read path. Don't point new integrations here. |
